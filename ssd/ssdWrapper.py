...
#-----------------------------------------------------------#
#   使用此处的接口进行训练，可以撇除shape的影响，可以接收其他数量类别（比如20类的预训练模型）的模型进行迁移学习
#-----------------------------------------------------------#
def getSSDModel(phase, num_classes, confidence=0.5, nms_iou=0.45):
    if phase == 'train':
        return ssd.get_ssd('train', num_classes, confidence, nms_iou)
    elif phase == 'test':
        return SSD()