...
from ssd.vggNet import vgg as add_vgg
from ssd.ssd_layers import Detect, L2Norm, PriorBox

class SSD(nn.Module):
    def __init__(self, phase, base, extras, head, num_classes, confidence, nnms_iou):
        super(SSD, self).__init__()
        self.phase = phase
        self.num_classes = num_classes
        self.cfg = Config
        # using the VGG as the base for future training
        ...
        if phase == 'test':
            ...
            self.detect = Detect(num_classes, 0, 200, confidence, nnms_iou)
    ...
    def forward(self, x):
        ...
        #   获得conv4_3的内容
        #   shape为38,38,512
        #   因为每一层Conv2d都需要额外一个ReLU，所以是对于con4_3 则是 23
        #---------------------------#
        for k in range(23):
            x = self.vgg[k](x)
